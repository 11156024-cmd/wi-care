      ...
      <div className="bg-white w-full max-w-xs mx-4 mb-4 sm:mb-0 rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 duration-300 border border-slate-100">
        ...
        <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-slate-700">Á≥ªÁµ±?Ä??/h2>
          </div>
          <button 
            onClick={onClose}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className={`font-bold text-sm ${esp32Connected ? 'text-green-700' : 'text-red-700'}`}>
                    {esp32Connected ? 'Â∑≤ÈÄ?é• ESP32-S3' : 'ESP32-S3 ?¢Á?'}
                  </p>
                  <p className="text-xs text-slate-500">
                    {esp32Connected ? 'IP: 172.20.10.9:8080' : 'Ë´ãÊ™¢?•Ë®≠?ôÈõªÊ∫êË?Á∂≤Ë∑Ø'}
                  </p>
                </div>
                {esp32Connected ? (
          ...
          <div>
            ...
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-blue-700 text-sm">ESP32 ?üÂØ¶?üÊ∏¨??/p>
                  <p className="text-xs text-slate-500">CSI WiFi ?ï‰??µÊ∏¨ÔºàÁÑ°Ê®°Êì¨?∏Ê?Ôº?/p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="px-5 py-3 bg-slate-50 text-center border-t border-slate-100">
            ...
            <p className="text-[10px] text-slate-300 font-mono mt-0.5">ESP32-S3 | ÂØ¶Áõ§Ê®°Â? | ?°Â??∏Ê?</p>
        </div>
