...
const CaregiverProfileView: React.FC<CaregiverProfileViewProps> = ({ isOpen, onClose }) => {
  ...
  const handleTestLine = async () => {
    if (!profile.lineChannelToken || !profile.lineUserId) {
        alert("Ë´ãÂ?Ëº∏ÂÖ• Channel Token ??User ID");
        return;
    }
    ...
    if (success) {
        alert("Ê∏¨Ë©¶?ºÈÄÅÊ??üÔ?\n(Ë´ãÈ???Console ?•Á?Ê®°Êì¨??API Ë´ãÊ?)");
    } else {
        alert("?ºÈÄÅÂ§±?óÔ?Ë´ãÊ™¢?•Ë®≠ÂÆö„Ä?);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-slate-50 flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="bg-white px-4 py-3 shadow-sm border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
        ...
        <h1 className="font-bold text-lg text-slate-800">?ßÈ°ß?ÖË???/h1>
        <div className="w-10" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-md mx-auto space-y-8">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="mt-3 text-sm text-slate-500">ÈªûÊ??¥Ê?Â§ßÈ†≠Ë≤?/p>
          </div>
            ...
            <div className="p-4 bg-slate-50/50">
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                    ...
                    <span className="underline">Â¶Ç‰?Ë®≠Â?Ôº?/span>
                </button>
              </div>
              ...
              {showLineHelp && (
                  <div className="mb-4 p-3 bg-blue-50 text-blue-800 text-xs rounded-lg leading-relaxed animate-in fade-in zoom-in-95 duration-200 border border-blue-100">
                      <p className="font-bold mb-1">Ë´ãÂ?ÂæÄ <a href="https://developers.line.biz/console/" target="_blank" className="underline text-blue-600">LINE Developers Console</a>Ôº?/p>
                      <ul className="list-decimal pl-4 space-y-1 opacity-90">
                          <li>Âª∫Á?‰∏Ä??Provider ??Messaging API Channel??/li>
                          <li>??<strong>Messaging API</strong> ?ÜÈ?ÔºåÁî¢??<strong>Channel access token</strong>??/li>
                          <li>??<strong>Basic settings</strong> ?ÜÈ?ÔºåÊç≤?∞Â??®Êâæ??<strong>Your user ID</strong> (?ãÁôº?ÖÂ?????/li>
                      </ul>
                      <p className="mt-2 text-[10px] text-blue-600/70">* Ë®ªÔ?Ê≠??‰∏äÁ??Ä?èÈ? Webhook ?≤Â?‰∏Ä?¨Áî®??ID??/p>
                  </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <div>
                   ...
                   <textarea
                        ...
                        placeholder="Ë´ãË≤º‰∏äÈï∑‰∏≤Á? Access Token..."
